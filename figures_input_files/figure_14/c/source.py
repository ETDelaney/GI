...
def space_distribution(plot=0):
	"""
	S, indeces = space_distribution(plot=0)

	Spatial frequency-domain power-spectral density distribution mask of the noise sources. 

	INPUT:
	------
	plot:		Plot source distribution when plot==1.
	...
	indeces:	Indeces where S is non-zero.
	...
	#- Single point source. -------------------------------------------------------
		...
		indeces=[]
		indeces.append((80,300))
		indeces.append((130,280))
	...
	#- Single point source on homogeneous background. -----------------------------
		...
		S=1000.0*np.ones(np.shape(x))

		S[80,300]=1.0e6
		S[130,280]=1.0e6

		#- Assign only the indeces of point sources for precompute in 'combined' mode.
		indeces=[]
		indeces.append((80,300))
		indeces.append((130,280))
	...
	#- Two lines of receivers (used for figure 1). --------------------------------
		...
		for i in range(20,80):
			S[i,300]=1.0e6
			indeces.append((i,300))

		for i in range(10,40):
			S[i,100]=1.0e6
			indeces.append((i,100))
	...
	#- Nearly equally distributed quasi-random sources distribution. --------------
		...
		for k in range(100):
			idx=np.floor(Nx*float(np.random.rand(1)))
			idy=np.floor(Ny*float(np.random.rand(1)))

			S[idy,idx]=1.0e6
			indeces.append((idy,idx))
	...
	#- Biased random distribution. ------------------------------------------------
		...
		for k in range(200):
			idx=np.floor(Nx*float((0.20+0.15*np.random.rand(1))))
			idy=np.floor(Ny*float((0.10+0.30*np.random.rand(1))))
...
def frequency_distribution(f,plot=0):
	"""
	...
	natural=np.zeros(np.shape(f))
	instrument=np.zeros(np.shape(f))
	...
	if p.instrument=='bandpass':
		...
		Nminmax=np.round((p.bp_fmin-np.min(f))/df)
		Nminmin=np.round((p.bp_fmin-p.bp_width-np.min(f))/df)
		Nmaxmin=np.round((p.bp_fmax-np.min(f))/df)
		Nmaxmax=np.round((p.bp_fmax+p.bp_width-np.min(f))/df)
	...
	if p.natural=='bandpass':
		...
		Nminmax=np.round((p.bp_fmin-np.min(f))/df)
		Nminmin=np.round((p.bp_fmin-np.min(f)-p.bp_width)/df)
		Nmaxmin=np.round((p.bp_fmax-np.min(f))/df)
		Nmaxmax=np.round((p.bp_fmax-np.min(f)+p.bp_width)/df)
	...
	if plot==1:
		...
		plt.loglog(T,natural,'k',linewidth=2)
		plt.title('source spectrum')
		plt.xlabel('period [s]')
		plt.ylabel('source power-spectral density [1 /s^2]')
		plt.show()

		plt.semilogx(T,instrument,'k',linewidth=2)
		plt.title('instrument response')
		plt.xlabel('period [s]')
		plt.ylabel('instrument response [1]')
		plt.show()

		plt.semilogx(T,np.sqrt(natural),'r')
